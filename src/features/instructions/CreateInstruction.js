  ...
  useEffect(() => {
    axios.get(`/users`).then((res) => {
      const fileteredUsers = res.data.filter(
        (user) => user.role === "Staff" && user._id !== auth?.userId
      );
  ...
  return (
    <div className="p-3 rounded bg-white shadow-sm">
      <div className="card text-bg-light d-flex flex-row gap-2 p-3 text-secondary">
        ...
        This is a form which allows you to send an instruction to a fellow
        academic staff member. You must provide all the details required by the
